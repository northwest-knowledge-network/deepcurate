 ...
 *  Under MIT License
 *
 * MODIFIED BY SHENEMAN (sheneman@uidaho.edu)
 * 
 */
var LenaJS = {};
...
LenaJS.filterImage2 = function(selector, filter, param, image) {

  var args = [this.getImage(image), param];

  return this.printCanvas(selector, filter.apply(null, args));
};

LenaJS.filterImage3 = function(selector, filter, param, can) {

  var args = [this.getImage3(can), param];
...
LenaJS.saturation = function(pixels,level) {
  //var level = 2.9,
    console.log("LenaJS.saturation.  level=%f", level);
    RW = 0.3086,
    RG = 0.6084,
    RB = 0.0820,
...
LenaJS.sepia = function(pixels, level) {

  console.log("sepia: level = %f", level);
